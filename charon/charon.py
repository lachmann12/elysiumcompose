...
expiration = int(os.environ['EXPIRATION_MINUTES'])
...
def createUser(username, password, firstname, lastname, email, self):
    ...
    if cur.rowcount > 0:
        ...
        query = "INSERT INTO userinfo (username, firstname, lastname, password, salt, email, role, uuid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        cur.execute(query, (username, firstname, lastname, saltpass, salt, email, "user", uid, ))
        db.commit()
        cur.close()
        db.close()
...
def listFiles(username, password, prefix, self):
    self.set_header("Access-Control-Allow-Origin", "*")
    db = getConnection()
    cur = db.cursor()
    query = "SELECT * FROM userinfo WHERE username=%s"
    ...
    if(saltpass == sp):
        ...
        bucket = conn.get_bucket(bucketname)
        for file in bucket.list(uid+"/"+prefix, "/"):
...
class AdminHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        if(saltpass == sp and role == "admin"):
            ...
            for res in cur:
                query = "SELECT username, firstname, lastname, email, date, uuid FROM jobqueue WHERE userid=%s"
                cur2 = db.cursor()
                cur2.execute(query, (res[6],))
                cur2.close()
...
class IndexHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        self.write(loader.load("charon_template.html").generate(charon_base=host_charon+"/"+entrypoint_charon))


application = tornado.web.Application([
    (r"/"+entrypoint_charon+"/sign_auth", SignAuth),
    (r"/"+entrypoint_charon+"/version", VersionHandler),
    (r"/"+entrypoint_charon+"/admin", AdminHandler),
    (r"/"+entrypoint_charon+"/files", ListHandler),
    (r"/"+entrypoint_charon+"/delete", DeleteHandler),
    (r"/"+entrypoint_charon+"/createuser", CreateUserHandler),
    (r"/"+entrypoint_charon+"/signpolicy", SignHandler),
    (r"/"+entrypoint_charon+"/login", LoginHandler),
    (r"/"+entrypoint_charon+"/index.html", IndexHandler),
    (r"/"+entrypoint_charon+"/(.*)", tornado.web.StaticFileHandler, dict(path=root))
])

ec2t = threading.Thread(target=s3thread, args=(awsid, awskey, fileage, bucketname, ))
ec2t.start()
