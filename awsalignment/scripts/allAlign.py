...
elysium_url = os.environ['ELYSIUM']
bucket_prerequisits = os.environ['BUCKET_PREREQUISITS']
...
r = requests.get(elysium_url+"/givejob?pass="+cloudpass)
jj = r.json()
...
if jj['id'] != "empty":
    if str(jj['type']) == "sequencing":
        ...
        indexlink = "https://s3.amazonaws.com/"+bucket_prerequisits+"/"+organism+"_index.idx"
        ...
        if not os.path.isfile(mapping):
            ...
            mappinglink = "https://s3.amazonaws.com/"+bucket_prerequisits+"/"+organism+"_mapping.rda"
        ...
        r = requests.get(elysium_url+'/finishjob?pass='+cloudpass+'&uid='+str(jj['uid']))
